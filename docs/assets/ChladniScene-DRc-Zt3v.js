import{n as e,r as t,t as n}from"./index-CM9IyRPz.js";import{a as r,c as i,d as a,f as o,g as s,h as c,i as l,l as u,m as d,n as f,o as p,p as m,r as h,s as g,t as _,u as v}from"./three-JsSwakKK.js";var y=class{kind=`cpu`;size;current;previous;next;constructor(e){this.size=e;let t=e*e;this.current=new Float32Array(t),this.previous=new Float32Array(t),this.next=new Float32Array(t),this.seed()}async step(r){let{settings:i,bands:a,time:o}=r,s=this.size,c=this.size,l=Math.PI*2*i.frequency;this.next.fill(0);for(let t=2;t<c-2;t+=1)for(let r=2;r<s-2;r+=1){let u=t*s+r,d=this.current[u]??0,f=this.current[u-s]??0,p=this.current[u+s]??0,m=this.current[u+1]??0,h=this.current[u-1]??0,g=this.current[u-s+1]??0,_=this.current[u-s-1]??0,v=this.current[u+s+1]??0,y=this.current[u+s-1]??0,b=this.current[u-2*s]??0,x=this.current[u+2*s]??0,S=this.current[u+2]??0,C=this.current[u-2]??0,w=20*d-8*(f+p+m+h)+2*(g+_+v+y)+b+x+S+C,T=r/(s-1),E=t/(c-1),D=e(T,E,i.modeX,i.modeY)+a.bass*e(T,E,1,1)+a.mid*e(T,E,2,3)+a.high*e(T,E,5,4),O=i.drive*i.audioGain*D*Math.sin(l*o),k=d-(this.previous[u]??0);this.next[u]=n(2*d-(this.previous[u]??0)+k*(1-i.damping)-i.stiffness*w+O,-1,1)}let u=this.previous;return this.previous=this.current,this.current=this.next,this.next=u,{field:this.current.slice(),energy:t(this.current)}}dispose(){this.current=new Float32Array,this.previous=new Float32Array,this.next=new Float32Array}seed(){for(let t=0;t<this.size;t+=1)for(let n=0;n<this.size;n+=1){let r=t*this.size+n,i=.003*e(n/(this.size-1),t/(this.size-1),2,3);this.current[r]=i,this.previous[r]=i}}},b=`
struct Params {
  width: f32,
  height: f32,
  stiffness: f32,
